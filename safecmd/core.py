...
Docs: https://AnswerDotAI.github.io/safecmdcore.html.md"""

# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/01_core.ipynb.

# %% auto #0
__all__ = ['default_cfg', 'cfg_path', 'ok_dests', 'ok_cmds', 'warn_cmds', 'run', 'CmdSpec', 'parse_cfg', 'validate_cmd',
           'DisallowedError', 'DisallowedCmd', 'DisallowedDest', 'normalize_dest', 'validate_dest', 'validate',
           'safe_run', 'bash', 'unsafe_bash', 'add_allowed_cmds', 'add_allowed_dests', 'rm_allowed_cmds',
           'rm_allowed_dests', 'ex', 'ex_str', 'sed', 'rg', 'main']
...
def _validate_dest_args(toks, cmds, dests):
    ...
    for pos in spec.dest_pos:
        try: d = args[pos]
        except IndexError: raise DisallowedCmd(toks, toks[0] if toks else None) from None
...
def validate(
    ...
    for c in commands:
        if not validate_cmd(c, cmds): raise DisallowedCmd(c, c[0] if c else None)
...
def bash(
    ...
    try: res = safe_run(cmd, rm_cmds=rm_cmds, rm_dests=rm_dests)
        ...
        if e.name in warn_cmds: return f"warning: {e.name} not recommended here. Command NOT run. Review available tools carefully and try something more suitable than this bash CLI."
        eff_cmds, eff_dests = _eff_sets(rm_cmds=rm_cmds, rm_dests=rm_dests)
        e.add_note(f'''\
allowed_cmds: {joins('; ', eff_cmds)}
suggestion: rerun using an allowed cmd, or ask user to provide permission''')
        raise
    except PermissionError as e:
        eff_cmds, eff_dests = _eff_sets(rm_cmds=rm_cmds, rm_dests=rm_dests)
        e.add_note(f'''\
allowed_dests: {joins('; ', eff_dests)}
suggestion: rerun using an allowed dest, or ask user to provide permission''')
        raise
    return res
...
def rg(args:str, rm_cmds:str=None, rm_dests:str=None):
