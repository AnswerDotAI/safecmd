...
Docs: https://AnswerDotAI.github.io/safecmd/bashxtract.html.md"""
...
# %% ../nbs/00_bashxtract.ipynb #e6bb5d04
_op_samples = dict(bin=['&&','||','|','|&'], redir=['>','>>','<','<&','>&','&>','&>>','<<<'])

_op_map = _redir_ops = _write_ops = None

def _walk(x, fn):
    if isinstance(x, dict):
        yield from fn(x)
        for v in x.values(): yield from _walk(v, fn)
    elif isinstance(x, list):
        for v in x: yield from _walk(v, fn)

def _find_op(typ): return lambda n: [n['Op']] if n.get('Type')==typ and 'Op' in n else []
def _find_redir(n): return [r['Op'] for r in n.get('Redirs', []) if r.get('Op') is not None]

def _ensure_ops():
    global _op_map, _redir_ops, _write_ops, _hstr_op
    if _op_map is not None: return
    _op_map = {next(_walk(parse_bash(f'a {s} b'), _find_op('BinaryCmd'))): s for s in _op_samples['bin']}
    rm = {next(_walk(parse_bash(f'a {s} b'), _find_redir)): s for s in _op_samples['redir']}
    _hstr_op = next(k for k,v in rm.items() if v=='<<<')
    _redir_ops = {k:v for k,v in rm.items() if v!='<<<'}
    _write_ops = {k:v for k,v in _redir_ops.items() if v in ('>','>>','&>','&>>')}
...
def visit_stmts(stmts, cmd, commands=None):
    ...
    for s in stmts or []:
        ...
        for r in s.get('Redirs', []):
            ...
            elif r.get('Op') == _hstr_op: commands[-1].extend(['<<<', word_text(r['Word'], cmd)])
    return commands
...
def collect_ops(node, ops=None):
    ...
    for r in node.get('Redirs', []):
        if op := _redir_ops.get(r.get('Op')): ops.add(op)
