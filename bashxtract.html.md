...
We need to know the codes for each op, so we look them up once and store
them:
...
<a
href="https://github.com/AnswerDotAI/safecmd/blob/main/safecmd/bashxtract.py#L127"
target="_blank" style="float:right; font-size:smaller">source</a>
...
scans a list of extracted commands for special flags and positional args
that represent commands or destinations. It takes four dicts mapping
command names to sets of flags/positions:
...
- `dest_flags`: flags whose next arg is a destination to validate (e.g.,
  `{'curl': {'-o', '--output'}}`)
- `dest_pos`: positional arg indices (0-based, after the command name)
  that are destinations (e.g., `{'ex': {0}, 'cp': {-1}}`). Negative
  indices work as in Python (e.g., `-1` for last arg).
- `exec_pos`: positional arg indices (0-based, after the command name)
  that are commands (e.g., `{'env': {0}, 'xargs': {0}}`).

Returns `(extra_cmds, extra_dests)` where `extra_cmds` is a list of
command strings to parse recursively (from both `exec_flags` and
`exec_pos`), and `extra_dests` is a list of `(flag_or_idx, dest)` tuples
to validate as redirect destinations.
...
`dest_pos` extracts positional arguments as destinations. Index `0`
grabs the first arg (e.g., `ex somefile`), and `-1` grabs the last
(e.g., `cp src dest`). Commands not in `dest_pos` are unaffected.

`exec_pos` works the same way but for commands. Index `0` grabs the
first arg as a command to validate recursively (e.g., `env ls` or
`xargs grep`):
...
<a
href="https://github.com/AnswerDotAI/safecmd/blob/main/safecmd/bashxtract.py#L162"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/safecmd/blob/main/safecmd/bashxtract.py#L171"
target="_blank" style="float:right; font-size:smaller">source</a>
...
`extract_commands(cmd, exec_flags=None, dest_flags=None, dest_pos=None, exec_pos=None)`
parses a bash command string and returns a 3-tuple of: 1. A list of all
commands that would be executed (including nested ones) 2. A set of
operators used in the command 3. A list of write redirect destinations
as `(op, dest)` tuples
...
(e.g., `{'curl': {'-o'}}`). These are added to redirects. - `dest_pos`:
dict mapping command names to sets of positional arg indices (0-based,
after the command) that are destinations (e.g.,
`{'ex': {0}, 'cp': {-1}}`). Negative indices work as in Python. -
`exec_pos`: dict mapping command names to sets of positional arg indices
that are commands (e.g., `{'env': {0}, 'xargs': {0}}`). These are parsed
recursively and added to commands.
...
`('-o', '/tmp/f')` to redirects - Dest positions: `ex somefile` with
`dest_pos={'ex': {0}}` → adds `(0, 'somefile')` to redirects - Exec
positions: `env ls` with `exec_pos={'env': {0}}` → adds `['ls']` to
commands
